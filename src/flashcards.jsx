...
import {normalizeCard,cleanGuess,reviewStats,dueCards} from './flashcard-model';
...
export default function Flashcards({onRequireOwner,onSuccess,ownerOpen}){
 ...
 const [flipped,setFlipped]=useState(false),[index,setIndex]=useState(0),[query,setQuery]=useState(''),[tag,setTag]=useState('all'),[all,setAll]=useState(false),[now,setNow]=useState(Date.now),[streak,setStreak]=useState(0);
 ...
 const filtered=useMemo(()=>cards.filter(c=>(tag==='all'||c.tag===tag)&&`${c.word} ${c.meaning} ${c.tag}`.toLowerCase().includes(query.toLowerCase())),[cards,tag,query]);
 ...
 useEffect(()=>{setFlipped(false);setGuess('');setGuessResult('')},[current?.id,all,tag]);
 ...
 return <section className="fcPage">
  ...
  {tab!=='progress'&&<div className="fcFilters"><label>หมวดคำศัพท์<select value={tag} onChange={e=>{setTag(e.target.value);setIndex(0)}}><option value="all">ทุกหมวด</option>{tags.map(t=><option key={t}>{t}</option>)}</select></label>{tab==='words'&&<label>ค้นหา<input value={query} onChange={e=>setQuery(e.target.value)} placeholder="คำศัพท์ หรือคำแปล"/></label>}{tab==='study'&&<button aria-pressed={all} onClick={()=>{setAll(v=>!v);setIndex(0);setQuery('')}}>{all?'แสดงเฉพาะคำถึงรอบ':'ทบทวนทั้งหมด'}</button>}</div>}
  ...
  {editor&&<CardEditor key={editor.id} initial={editor} onClose={()=>setEditor(null)} onSave={saveCard} busy={busy}/>}
 </section>;
}
...
function CardEditor({initial,onClose,onSave,busy}){const [draft,setDraft]=useState(initial),[file,setFile]=useState(null),[saving,setSaving]=useState(false),[error,setError]=useState('');const first=useRef(null),dialog=useRef(null);useEffect(()=>{const previous=document.activeElement;first.current?.focus();return()=>previous?.focus()},[]);const disabled=busy||saving;return <div className="fcModalBackdrop" onKeyDown={e=>{if(e.key==='Escape'&&!disabled)onClose();if(e.key==='Tab'){const nodes=[...dialog.current.querySelectorAll('button:not(:disabled),input:not(:disabled),textarea:not(:disabled)')];if(e.shiftKey&&document.activeElement===nodes[0]){e.preventDefault();nodes.at(-1)?.focus()}else if(!e.shiftKey&&document.activeElement===nodes.at(-1)){e.preventDefault();nodes[0]?.focus()}}}}><form ref={dialog} className="fcModal" role="dialog" aria-modal="true" aria-labelledby="fc-editor-title" onSubmit={async e=>{e.preventDefault();if(disabled)return;setSaving(true);setError('');try{const image=file?await prepareImage(file):null;await onSave(draft,image)}catch(err){setError(err.message)}finally{setSaving(false)}}}><header><h2 id="fc-editor-title">บันทึกคำศัพท์</h2><button type="button" aria-label="ปิด" disabled={disabled} onClick={onClose}>×</button></header><fieldset disabled={disabled}>{[['word','คำศัพท์ *'],['meaning','คำแปล *'],['phonetic','คำอ่าน'],['tag','หมวด'],['example','ประโยคตัวอย่าง'],['translation','คำแปลประโยค'],['imageUrl','ลิงก์รูปช่วยจำ (ไม่บังคับ)']].map(([key,label])=><label key={key}>{label}<input ref={key==='word'?first:undefined} value={draft[key]} onChange={e=>setDraft({...draft,[key]:e.target.value})} required={['word','meaning'].includes(key)} maxLength={key==='example'||key==='translation'?2000:1000} type={key==='imageUrl'?'url':'text'}/></label>)}<label>หรืออัปโหลดภาพช่วยจำ<span className="fcFileBtn"><span>{file?file.name:'เลือกรูปจากเครื่อง'}</span><input type="file" accept="image/png,image/jpeg,image/webp" onChange={e=>setFile(e.target.files[0]||null)}/></span><small>รูปที่เลือกจะเก็บใน Google Drive และใช้เป็นภาพคำใบ้</small></label>{error&&<p role="alert">{error}</p>}<button className="fcPrimary" disabled={disabled}>{disabled?'กำลังบันทึก…':'บันทึกคำศัพท์'}</button></fieldset></form></div>}