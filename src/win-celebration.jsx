...
export function WinCelebration({onClose,label='SMALL WIN • BIG JOY',title='เก่งมาก วันนี้ทำได้แล้ว!',message,actionLabel='ไปต่อกันเลย!'}){
 ...
 return createPortal(<div className="winVictory"><div className="winVictoryCard">
  ...
  <div role="status" aria-live="polite"><p className="winVictoryLabel">{label}</p><h2>{title}</h2><p>{message||<>เก็บความสำเร็จลง Sheet แล้ว<br/>อีกหนึ่งก้าวที่น่าภูมิใจของเรา</>}</p></div>
  <button type="button" className="winVictoryContinue" onClick={onClose}>{actionLabel}</button>
