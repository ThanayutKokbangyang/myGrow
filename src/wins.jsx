...
export function SmallWins({onSuccess}){
 const [wins,setWins]=useState(()=>{try{return normalizeWins(JSON.parse(localStorage.getItem(STORAGE)||'[]'));}catch{return [];}});
 ...
 const [editing,setEditing]=useState(null),[removing,setRemoving]=useState(null),[message,setMessage]=useState('');
 ...
 function persist(next){try{localStorage.setItem(STORAGE,JSON.stringify(next));setWins(next);return true}catch{setMessage('บันทึกไม่สำเร็จ กรุณาสำรองข้อมูลและตรวจพื้นที่เก็บข้อมูล');return false}}
 ...
 function save(e){e.preventDefault();if(!text.trim())return;const original=editing?wins.find(w=>w.id===editing):null;if(editing&&!original){setMessage('ไม่พบรายการเดิม');return;}
  ...
  if(persist(next)){setText('');setEditing(null);setToday(dayKey());selectDay(item.date);setMonth(parseDay(item.date));setFilter('all');setMessage('บันทึกความสำเร็จแล้ว เก่งมาก!');onSuccess();}
 }
  ...
  <div className="pixelWinsLayout">
   ...
   <div className="winsArchive">
    ...
    <section className="pixelWinList"><div className="winListHeading"><Trophy/><div><h2>{selected===today?'ความสำเร็จวันนี้':formatDate(selected)}</h2><p>{selectedWins.length} เรื่องดี ๆ ที่เก็บไว้</p></div></div>
     ...
     {shown.length===0?<div className="pixelWinsEmpty"><Trophy/><p>{selectedWins.length?'ยังไม่มีบันทึกในหมวดนี้':'ยังไม่มีความสำเร็จในวันนี้'}</p><small>เขียนเรื่องดี ๆ ของวันนี้ได้ในช่องบันทึก</small></div>:shown.map(w=>{const c=categoryOf(w.category);return <article key={w.id} style={{'--category':c.color}}><div className="winEntryCategory"><WinIcon name={c.icon}/>{c.label}</div><p>{w.text}</p><div className="winEntryActions">{removing===w.id?<><span>ลบรายการนี้?</span><button type="button" onClick={()=>{if(persist(wins.filter(x=>x.id!==w.id))){setRemoving(null);if(editing===w.id){setEditing(null);setText('')}setMessage('ลบรายการแล้ว')}}}>ยืนยันลบ</button><button type="button" onClick={()=>setRemoving(null)}>ยกเลิก</button></>:<><button type="button" onClick={()=>{setEditing(w.id);setText(w.text);setCategory(c.id);textRef.current?.focus();textRef.current?.scrollIntoView({behavior:'smooth',block:'center'})}}>แก้ไข</button><button type="button" onClick={()=>setRemoving(w.id)}>ลบ</button></>}</div></article>})}
    </section>
   </div>
  </div>
  <p className="winMessage" role="status">{message}</p>
  <footer><small>เก็บในเบราว์เซอร์เครื่องนี้ · สำรองข้อมูลไว้ได้เสมอ</small><button type="button" onClick={()=>{const url=URL.createObjectURL(new Blob([JSON.stringify(wins,null,2)],{type:'application/json'}));const a=document.createElement('a');a.href=url;a.download=`small-wins-${dayKey()}.json`;a.click();setTimeout(()=>URL.revokeObjectURL(url),1000)}}>สำรองข้อมูล</button><label className="importWins">นำเข้าข้อมูล<input type="file" accept=".json,application/json" onChange={async e=>{const file=e.target.files[0];e.target.value='';if(!file)return;try{if(file.size>5000000)throw Error();const imported=normalizeWins(JSON.parse(await file.text()));const ids=new Set(wins.map(w=>w.id));const next=[...wins,...imported.filter(w=>!ids.has(w.id)&&ids.add(w.id))];if(persist(next))setMessage('นำเข้าข้อมูลเรียบร้อย')}catch{setMessage('ไฟล์ไม่ถูกต้อง กรุณาเลือกไฟล์สำรองความสำเร็จ')}}}/></label></footer>
 </section>;
}
